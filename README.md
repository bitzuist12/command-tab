...
### Native Google connectors (read-only)

Command Tab can read your **Calendar** and **Gmail** directly via its own OAuth flow — no external backend required. Add your own Google OAuth Desktop-app client (env vars or `command-tab-context/google-credentials.json`), then click **Connect** on the Calendar or Gmail card. Each authorizes separately; tokens are stored locally and gitignored; failures are shown, never faked.
